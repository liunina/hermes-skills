...
function patchOrchestratorWorkflow(workflow, aiTemplate) {
  ...
  nodeByName(patched, 'Attach final Wiki link').parameters.jsCode = "function ctx(){ try { const x=$('Attach HTML report link').first().json||{}; if(Object.keys(x).length) return x; } catch {} try { const x=$('Skip HTML publish').first().json||{}; if(Object.keys(x).length) return x; } catch {} return $('Format final report').first().json||{}; } const out=ctx(); const archive=$input.first().json||{}; const latestWikiLink='https://wiki.dinve.com/zh/'+String(out.finalWikiPath||out.wikiPath||'').replace(/^\\/+|\\/+$/g,''); const archiveWikiLink=archive.wikiLink||out.wikiArchiveLink||''; const latest={...($('Publish final Wiki report').first().json||{}),wikiLink:latestWikiLink}; let rowInput={}; try { rowInput=JSON.parse(out.row?.inputJson_object||'{}'); } catch {} rowInput.wikiArchive={wikiArchivePath:out.wikiArchivePath||'',wikiArchiveLink:archiveWikiLink,publish:archive}; const row={...(out.row||{}),inputJson_object:JSON.stringify(rowInput),finalWikiLink:latestWikiLink}; return [{json:{...out,row,wikiPublish:latest,wikiLink:latestWikiLink,wikiArchivePublish:archive,wikiArchiveLink:archiveWikiLink}}];";
