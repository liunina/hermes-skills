...
| M3 | 实现 `n8n-workflow-skills` MCP manager skeleton | Done |
| M4 | 增加 registry 校验、MCP smoke test 和 CI 集成 | Done |
| M5 | 文档补齐：安装、扩展、安全规则 | Done |
| M6 | 迁移首批组件 workflow skills：Wiki.js 发布、Mattermost 通知 | Done |
...
- [x] 创建 `mcp/n8n-workflow-skills/` MCP manager。
...
- [x] 运行本地验证。
- [x] 做敏感信息扫描。
- [x] 迁移组件 skill：`publish-markdown-to-wiki`。
- [x] 迁移组件 skill：`send-mattermost-notification`。
- [x] 增加 `sideEffectMode: "always"`，用于默认有副作用的组件 skill。
- [x] 重新运行 registry、skill、MCP、安装和敏感信息验证。

## 当前决策

- `workflow-registry/*.json` 是可公开的 capability manifest。
...
- `mcp/n8n-workflow-skills` 当前提供三个核心工具：`list_workflow_skills`、`get_workflow_skill`、`run_workflow_skill`。
...
- 组件类 workflow skill 使用 `sideEffectMode: "always"`，因为执行本身就是写入或发送动作。
...
- 2026-07-09：`node install.mjs --client generic --install-dir /tmp/hermes-n8n-workflow-skills-install-test` 通过。
- 2026-07-09：`skills/amazon-competitor-analysis` 通过 Codex skill quick validation。
- 2026-07-09：新增 `publish-markdown-to-wiki` 和 `send-mattermost-notification` 组件 skill，并通过 registry/MCP smoke test。
...
- 2026-07-09：`node smoke-test.mjs` 重新通过，确认 3 个 MCP tools、组件 skill 列表和 3 个 manifest 声明的 side-effect guard。
- 2026-07-09：`node mcp/n8n-workflow-skills/install.mjs --client generic --install-dir /tmp/hermes-n8n-workflow-skills-install-test` 重新通过。
- 2026-07-09：`skills/amazon-competitor-analysis`、`skills/publish-markdown-to-wiki`、`skills/send-mattermost-notification` 均通过 Codex skill quick validation。
